...
/* ── Mock auth ──────────────────────────────────────────────────────────────
 * Demo login accounts (mock mode only). Sign in with the email + password,
 * or the phone number + any 6-digit OTP.
 * ┌────────────┬──────────────────────┬─────────────┬───────────────┐
...
function mockJwt({ sub, role, fullName }) {
  const now = Math.floor(Date.now() / 1000);
  const header = b64url({ alg: 'none', typ: 'JWT' });
  const payload = b64url({ sub, role, fullName, iat: now, exp: now + 60 * 60 * 24 * 7 });
  return `${header}.${payload}.mock`;
}
function sessionFor(acc) {
  const userId = `demo_${acc.role.toLowerCase()}`;
  return {
    accessToken: mockJwt({ sub: userId, role: acc.role, fullName: acc.fullName }),
    userId,
    ...
    setTimeout(() => {
      try {
        if (path === '/auth/login' && options.method === 'POST') {
          ...
          const legacy = (id === '9876543210' || id === 'test@example.com') && password === 'password123';
          const match = DEMO_ACCOUNTS.find((a) => a.email === id || a.phone === id);

          if (legacy) {
            resolve(sessionFor({ role: 'CUSTOMER', fullName: 'Test User' }));
          } else if (match && password === DEMO_PASSWORD) {
            resolve(sessionFor(match));
          } else if (match) {
            reject(new Error('Wrong password for this demo account.'));
          } else {
            reject(new Error('No demo account for that email or number.'));
          }
        } else if (path === '/auth/register' && options.method === 'POST') {
          const body = JSON.parse(options.body);
          resolve(sessionFor({ role: 'CUSTOMER', fullName: body.fullName || 'New Customer' }));
          ...
          const { phone, otp } = JSON.parse(options.body);
          if (phone && otp) {
            const match = DEMO_ACCOUNTS.find((a) => a.phone === String(phone).replace(/\D/g, '').slice(-10));
            const session = sessionFor(match || { role: 'CUSTOMER', fullName: 'Existing User' });
            resolve({ ...session, isNewUser: false });
          } else {
